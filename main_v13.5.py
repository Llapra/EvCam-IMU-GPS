...
class IMUWorker(QThread):
    ...
    def _open_new_chunk(self):
        ...
        self.bin_file = open(self.current_chunk_path, "wb", buffering=0)
                        ...
                        while len(buffer) >= self.PACKET_SIZE:
                            ...
                            if header_check in (self.HEADER_IMU, self.HEADER_GPS, self.HEADER_COORD):
                                ...
                                if header_check == self.HEADER_IMU:
                                    if self.recording and self.bin_file: self.bin_file.write(data_packet)
                                    packet_counter_hz += 1
...
class MainWindow(QMainWindow):
    ...
    def create_controls_panel(self):
        ...
        self.cmb_chunk.addItem("10 Segundos (TEST)", 10) # Nueva opción para pruebas
        self.cmb_chunk.addItem("5 Minutos", 300)
        self.cmb_chunk.addItem("30 Minutos", 1800)
        self.cmb_chunk.addItem("1 Hora (Por defecto)", 3600)
    ...
    def plot_data(self):
        # 1. PREVENCIÓN DE FUGA DE MEMORIA EN MATPLOTLIB
        if hasattr(self, 'fig_imu'):
            plt.close(self.fig_imu) # Destruye la figura anterior en el backend global
        ...
        ax1 = self.fig_imu.add_subplot(gs[0, 0]) # Acelerómetro (Izquierda Arriba)

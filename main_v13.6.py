...
# --- GESTOR DE ESCRITURA ASÍNCRONA (Staging Area Architecture) ---
class DiskWriterWorker(threading.Thread):
    """
    Consumidor en segundo plano. Extrae archivos terminados del área
    de staging (SSD) y los transfiere al almacenamiento masivo (HDD).
    """
    def __init__(self):
    ...
    def enqueue_file(self, temp_path, final_dest_dir):
        """Añade un archivo a la cola de transferencia."""
        self.transfer_queue.put((temp_path, final_dest_dir))

    def run(self):
        while self.running:
            try:
                temp_path, final_dest_dir = self.transfer_queue.get(timeout=1.0)
                try:
                    os.makedirs(final_dest_dir, exist_ok=True)
                    final_path = os.path.join(final_dest_dir, os.path.basename(temp_path))
                    # shutil.move transfiere y elimina el archivo origen automáticamente
                    shutil.move(temp_path, final_path)
                    print(f"I/O Hilo: Archivo transferido a {final_path}")
                except Exception as e:
                    print(f"I/O Error moviendo archivo {temp_path}: {e}")
                finally:
                    self.transfer_queue.task_done()
            except queue.Empty:
                continue

# Instancia global de inicialización temprana
disk_writer = DiskWriterWorker()
disk_writer.start()
...
# Forzar PyQtGraph y ACTIVAR ACELERACIÓN OPENGL POR HARDWARE (GPU NVIDIA)
...
# --- CLASE DE VIDEO ACELERADA POR GPU (PyQtGraph) ---
class EvCamVisor(pg.GraphicsLayoutWidget):
    """Visor OpenGL para matrices NumPy de alta velocidad con ROI nativo"""
    ...
    def __init__(self):
        ...
        self.view.invertY(True) # Invertir eje Y para formato de imagen estándar
        
        # Item de imagen directo a GPU
        self.img_item = pg.ImageItem()
        self.view.addItem(self.img_item)
        
        # ROI Nativo de PyQtGraph
        self.roi = pg.RectROI([0, 0], [640, 480], pen=pg.mkPen('r', width=2))
        self.roi.addScaleHandle([1, 1], [0, 0])
        self.roi.addScaleHandle([0, 0], [1, 1])
        self.view.addItem(self.roi)
        self.roi.hide() # Oculto por defecto
        
        self.roi.sigRegionChanged.connect(self.calculate_sensor_roi)

    def calculate_sensor_roi(self):
        pos = self.roi.pos()
        size = self.roi.size()
        x = max(0, min(int(pos[0]), 639))
        y = max(0, min(int(pos[1]), 479))
        w = max(1, min(int(size[0]), 640 - x))
        h = max(1, min(int(size[1]), 480 - y))
    ...
    def set_image(self, img_array):
        # autoLevels=False evita que la CPU recalcule el contraste, enviando la matriz pura
        self.img_item.setImage(img_array, autoLevels=False)
...
class CameraWorker(QThread):
    ...
    def __init__(self, use_simulation=False):
        ...
        # --- VARIABLES DE CHUNKING Y STAGING ---
        self.req_start_rec = False
        self.req_stop_rec = False
        self.staging_dir = ""
        self.final_dest_dir = ""
        self.file_prefix = "EvCam"
        self.chunk_duration_s = 1800  # 30 minutos por defecto (1800 segundos)
    ...
    def start_recording(self, staging_dir, final_dest_dir, prefix, chunk_duration):
        self.staging_dir = staging_dir
        self.final_dest_dir = final_dest_dir
        self.file_prefix = prefix
        self.chunk_duration_s = chunk_duration # Actualizamos el tiempo aquí
        self.req_start_rec = True
        return True
        ...
        while self.running:
            try:
                ...
                for evs in mv_iterator:
                    ...
                    # --- LÓGICA DE CHUNKING CORREGIDA ---
                    # 1. Inicio de grabación inicial
                    if self.req_start_rec:
                        if i_events_stream:
                            timestamp = time.strftime("%Y%m%d_%H%M%S")
                            self.current_chunk_path = os.path.join(self.staging_dir, f"{self.file_prefix}_{timestamp}.raw")
                            i_events_stream.log_raw_data(self.current_chunk_path)
                            self.recording = True
                            self.chunk_start_time = now
                            print(f"DEBUG: Iniciando primer Chunk en {self.current_chunk_path}")
                        self.req_start_rec = False
                        
                    # 2. Disparo de nuevo Chunk (Rollover)
                    if self.recording and (now - self.chunk_start_time) >= self.chunk_duration_s:
                        if i_events_stream:
                            # CERRAMOS EL ACTUAL
                            i_events_stream.stop_log_raw_data()
                            
                            # NOTIFICAMOS AL DISCO (Transferencia asíncrona inmediata)
                            disk_writer.enqueue_file(self.current_chunk_path, self.final_dest_dir)
                            
                            # ABRIMOS EL SIGUIENTE SIN PERDER TIEMPO
                            timestamp = time.strftime("%Y%m%d_%H%M%S")
                            self.current_chunk_path = os.path.join(self.staging_dir, f"{self.file_prefix}_{timestamp}.raw")
                            i_events_stream.log_raw_data(self.current_chunk_path)
                            self.chunk_start_time = now
                            print(f"DEBUG: Rollover de Chunk ejecutado. Nuevo archivo: {self.current_chunk_path}")
                    
                    # 3. Parada solicitada por el usuario
                    ...
                    # --- 2. LÓGICA DE VISUALIZACIÓN ULTRA-MINIMALISTA ---
                    visual_period = 1.0 / self.target_fps
                    
                    if (now - last_visual_time) >= visual_period:
                        step = max(1, len(evs) // 10000)
                        events_to_process = evs[::step]
                        
                        if self.roi_enabled:
                            rx, ry, rw, rh = self.roi_coords
                            mask = (events_to_process['x'] >= rx) & (events_to_process['x'] < rx + rw) & \
                                   (events_to_process['y'] >= ry) & (events_to_process['y'] < ry + rh)
                            events_to_process = events_to_process[mask]

                        visual_buffer.fill(0) 
                        
                        on_mask = events_to_process['p'] == 1
                        off_mask = events_to_process['p'] == 0
                        
                        if np.any(on_mask):
                            visual_buffer[events_to_process['y'][on_mask], events_to_process['x'][on_mask]] = [0, 255, 0] 
                        if np.any(off_mask):
                            visual_buffer[events_to_process['y'][off_mask], events_to_process['x'][off_mask]] = [255, 0, 0]

                        # Pasamos la matriz NumPy directamente a la señal (Bypass QImage/CPU)
                        self.image_signal.emit(visual_buffer.copy())
                        last_visual_time = now
                        
                        event_rate = len(evs) / current_delta_t
                        if self.show_raw_data:
                            debug_text = f"[FÍSICA] Tasa: {event_rate:.2f} MEv/s\n[VISUAL] 1 de cada {step}\n"
                            if self.recording: 
                                debug_text += f"[REC] Escribiendo SSD (Chunking)...\nArchivo: {os.path.basename(self.current_chunk_path)}\n"
                            self.stats_signal.emit(debug_text, event_rate)
                        else:
                            self.stats_signal.emit("", event_rate)
                            
                        del events_to_process, on_mask, off_mask
...
class IMUWorker(QThread):
    ...
    def __init__(self, port, baudrate):
        ...
        self.staging_dir = ""
        self.final_dest_dir = ""
        self.file_prefix = "Ace"
        self.chunk_duration_s = 1800  # 30 minutos por defecto
        self.current_chunk_path = None
        self.chunk_start_time = 0
        # ---------------------------------------
        ...
        self.HEADER_ATM = 0xFFEE
        
        # --- BÚFER DE ESCRITURA EN MEMORIA ---
        self.write_buffer = bytearray()
        self.WRITE_CHUNK_SIZE = 8192  # Volcado a disco cada 8 KB
    ...
    def _open_new_chunk(self):
        if self.bin_file:
            ...
            if self.current_chunk_path:
                disk_writer.enqueue_file(self.current_chunk_path, self.final_dest_dir)
        
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        self.current_chunk_path = os.path.join(self.staging_dir, f"{self.file_prefix}_{timestamp}.bin")
        self.bin_file = open(self.current_chunk_path, "wb")
        self.chunk_start_time = time.time()

    def start_recording(self, staging_dir, final_dest_dir, prefix, raw_mode):
        self.staging_dir = staging_dir
        self.final_dest_dir = final_dest_dir
        self.file_prefix = prefix
        ...
        try:
            if self.raw_mode:
                # Abrimos el archivo único para toda la sesión
                timestamp = time.strftime("%Y%m%d_%H%M%S")
                self.current_chunk_path = os.path.join(self.staging_dir, f"{self.file_prefix}_{timestamp}.bin")
                self.bin_file = open(self.current_chunk_path, "wb")
                self.recording = True
    ...
    def stop_recording(self):
        ...
        self.armed = False
        if self.bin_file:
            # Vaciado seguro de memoria residual
            if self.write_buffer:
                try: self.bin_file.write(self.write_buffer)
                except: pass
    ...
    def run(self):
        try:
            ...
            while self.running:
                # El IMU ahora ignora el cronómetro de chunks para mantenerse en un solo archivo
                ...
                if ser.in_waiting:
                    ...
                    if not self.raw_mode:
                        while len(buffer) >= self.PACKET_SIZE:
                            ...
                            if header_check in (self.HEADER_IMU, self.HEADER_GPS, self.HEADER_COORD):
                                data_packet = buffer[:self.PACKET_SIZE]
                                
                                if header_check == self.HEADER_IMU:
                                    if self.recording and self.bin_file: 
                                        self.write_buffer.extend(data_packet)
                                    packet_counter_hz += 1
                                    ...
                                    if self.recording and self.bin_file: 
                                        self.write_buffer.extend(data_packet)
                                    self.pps_count += 1
                                    try: self.pps_signal.emit(self.pps_count)
                                    except: pass
                                    
                                elif header_check == self.HEADER_COORD:
                                    try:
                                        p = struct.unpack_from("<H f f H B B B B B B", data_packet)
                                        ...
                                        if self.armed and valid == 1:
                                            ...
                                            atm_packet = struct.pack("<H d 8x", self.HEADER_ATM, unix_ts)
                                            self.write_buffer.extend(atm_packet)
                                ...
                                # Lógica de escritura controlada (Volcado a disco)
                                if self.recording and len(self.write_buffer) >= self.WRITE_CHUNK_SIZE:
                                    self.bin_file.write(self.write_buffer)
                                    self.write_buffer.clear()
                                ...
                                # OPTIMIZACIÓN EXTREMA: Búsqueda rápida del próximo sincronizador
                                # Buscamos la huella en bytes de los Headers para saltar instantáneamente
                                idx_imu = buffer.find(b'\xaa\xbb', 1)
                                idx_gps = buffer.find(b'\xcc\xbb', 1)
                                idx_coord = buffer.find(b'\xdd\xbb', 1)
                                
                                valid_indices = [i for i in (idx_imu, idx_gps, idx_coord) if i != -1]
                                
                                if valid_indices:
                                    next_start = min(valid_indices)
                                    sync_errors += next_start
                                    del buffer[:next_start] # Salto cuántico, sin iterar
                                else:
                                    # Descartamos todo menos el último byte por si es la mitad de un header
                                    sync_errors += len(buffer) - 1
                                    del buffer[:-1]
...
class ReconWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # 1. ASIGNACIÓN ESTÁTICA DE MEMORIA (Previene el colapso del Garbage Collector)
            log_i = np.zeros((self.height, self.width), dtype=np.float32)
            intensity = np.zeros((self.height, self.width), dtype=np.float32)
            
            last_visual_time = time.time()
            visual_period = 1.0 / 30.0 # Limitamos el renderizado GUI a 30 FPS constantes

            for evs in mv_iterator:
                if not self.running: break
                
                # 2. FÍSICA: DECAIMIENTO CONSTANTE (Se ejecuta en cada iteración)
                ...
                # 3. RENDERIZADO VISUAL OPTIMIZADO (Separado de la integración física)
                if (now - last_visual_time) >= visual_period:
                    # Aplicamos Exponencial "In-place" sobrescribiendo la matriz de intensidad
                    np.exp(log_i, out=intensity)

                    # Extraemos una matriz reducida (1/16 de tamaño) para el cálculo de percentiles
                    sub_intensity = intensity[::4, ::4]
                    p_low, p_high = np.percentile(sub_intensity, (1, 99))
                    ...
                    # Estructuramos a 3 canales y emitimos matriz nativa a la GPU
                    rgb_img = np.stack((norm_img,)*3, axis=-1)
                    self.image_signal.emit(rgb_img)
                    last_visual_time = now
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("EvCam-IMU-GPS  -  Lab Control")
        self.setGeometry(50, 50, 1300, 700)
        
        # --- GESTIÓN DE DIRECTORIOS (Staging Area) ---
        # 1. Almacenamiento Final (Destino lento, ej. HDD Externo)
        self.base_save_dir = os.path.join(os.getcwd(), "Grabaciones_Finales")
        
        # 2. Área de Preparación / Staging (Almacenamiento rápido, SSD interno)
        self.staging_dir = os.path.join(tempfile.gettempdir(), "EvCam_Staging")
        ...
        self.imu_worker_1 = None
        self.imu_worker_2 = None

        # Variables internas para la última coordenada de cada ESP32
        self.latest_lat_1 = None; self.latest_lon_1 = None
        self.latest_lat_2 = None; self.latest_lon_2 = None

    def refresh_cameras(self):
        """Consulta al bus USB por los números de serie reales de las cámaras de eventos."""
        self.cmb_cam_1.clear()
        self.cmb_cam_2.clear()
        if hasattr(self, 'cmb_cam_recon'): self.cmb_cam_recon.clear()
        
        if HAS_METAVISION:
            try:
                sources = DeviceDiscovery.list_serial_numbers()
                if not sources:
                    self.txt_log.append("⚠ No se detectaron cámaras Metavision en el bus USB.")
                else:
                    self.cmb_cam_1.addItems(sources)
                    self.cmb_cam_2.addItems(sources)
                    if hasattr(self, 'cmb_cam_recon'): self.cmb_cam_recon.addItems(sources)
                    self.txt_log.append(f"✓ Cámaras detectadas (N° Serie): {sources}")
            except Exception as e:
                self.txt_log.append(f"❌ Error consultando HAL: {e}")
        else:
            self.txt_log.append("⚠ Librerías Metavision no encontradas. Usando Simulador.")

    def select_save_directory(self):
        """Define el punto de montaje final en el sistema de archivos (HDD Externo)."""
        directory = QFileDialog.getExistingDirectory(self, "Seleccionar destino final (HDD Externo)")
        if directory:
            self.base_save_dir = directory
            self.lbl_save_dir.setText(f"Destino (HDD): {self.base_save_dir}\nStaging (SSD): {self.staging_dir}")
            self.txt_log.append(f"✓ Destino de volcado configurado: {self.base_save_dir}")
    ...
    def create_visor_panel(self):
        ...
        btn_refresh_cams = QPushButton("⟳ DETECTAR CÁMARAS EN BUS USB")
        btn_refresh_cams.setStyleSheet("background-color: #00aaaa; color: black; font-weight: bold; padding: 5px;")
        btn_refresh_cams.clicked.connect(self.refresh_cameras)
        main_layout.addWidget(btn_refresh_cams)

        row_cam1 = QHBoxLayout()
        ctrl_cam1 = QGroupBox("CÁMARA 1 (Arriba)")
        l_ctrl1 = QVBoxLayout()
        self.cmb_cam_1 = QComboBox()
        ...
        l_ctrl1.addWidget(QLabel("Cámara (HAL Serial):"))
        l_ctrl1.addWidget(self.cmb_cam_1)
        ...
        l_ctrl2.addWidget(QLabel("Cámara (HAL Serial):"))
        l_ctrl2.addWidget(self.cmb_cam_2)
    ...
    def create_controls_panel(self):
        ...
        self.sl_fps.setRange(5, 165); self.sl_fps.setValue(20) # Modificado a 20 FPS por defecto
        self.sb_fps = QSpinBox()
        self.sb_fps.setRange(1, 200); self.sb_fps.setValue(20); self.sb_fps.setFixedWidth(100)
        ...
        self.btn_set_dir = QPushButton("📁 ASIGNAR DESTINO FINAL")
        self.btn_set_dir.clicked.connect(self.select_save_directory)
        self.btn_set_dir.setStyleSheet("background-color: #d1861a; color: white; font-weight: bold;")
        self.lbl_save_dir = QLabel(f"Destino: {self.base_save_dir}\nStaging (SSD): {self.staging_dir}")
        self.lbl_save_dir.setWordWrap(True)
        self.lbl_save_dir.setStyleSheet("color: #aaa; font-size: 10px; border: 1px solid #444; padding: 2px;")
        l_ctrl.addWidget(self.btn_set_dir)
        l_ctrl.addWidget(self.lbl_save_dir)
        l_ctrl.addSpacing(10)
        
        l_ctrl.addWidget(QLabel("Duración de cada Bloque (Chunk):"))
        self.cmb_chunk = QComboBox()
        self.cmb_chunk.addItem("10 Segundos", 10)
        self.cmb_chunk.addItem("2 Minutos", 120)
        self.cmb_chunk.addItem("5 Minutos", 300)
        self.cmb_chunk.addItem("10 Minutos", 600)
        self.cmb_chunk.addItem("15 Minutos", 900)
        self.cmb_chunk.addItem("30 Minutos", 1800)
        self.cmb_chunk.addItem("1 Hora", 3600)
        self.cmb_chunk.addItem("2 Horas", 7200)
        self.cmb_chunk.setCurrentIndex(3) # Ahora el índice 3 es "1 Hora"
        self.cmb_chunk.setStyleSheet("background-color: #333; color: #0f0; font-weight: bold; padding: 3px;")
        l_ctrl.addWidget(self.cmb_chunk)
        l_ctrl.addSpacing(5)
    ...
    def toggle_roi(self, checked):
        if checked:
            self.lbl_video_1.roi.show()
            self.lbl_video_2.roi.show()
        else:
            self.lbl_video_1.roi.hide()
            self.lbl_video_2.roi.hide()
            
        for worker, visor in [(self.worker_1, self.lbl_video_1), (self.worker_2, self.lbl_video_2)]:
            if worker:
                worker.roi_enabled = checked
                if checked: 
                    visor.calculate_sensor_roi()
                else: 
                    worker.roi_coords = (0, 0, 640, 480)
    ...
    def update_image(self, cam_id, img_array):
        if cam_id == 1: self.lbl_video_1.set_image(img_array)
        elif cam_id == 2: self.lbl_video_2.set_image(img_array)

    def update_recon_image(self, img_array):
        self.lbl_recon_video.set_image(img_array)
    ...
    def _start_camera(self, cam_id):
        ...
        if cam_id == 1:
            worker.serial_number = self.cmb_cam_1.currentText()
            ...
            worker.serial_number = self.cmb_cam_2.currentText()
        ...
        if self.chk_roi.isChecked(): visor_activo.calculate_sensor_roi()
    ...
    def toggle_recording(self):
        ...
        if not is_recording:
            # Creación dinámica de carpetas destino final (HDD)
            final_cam_dir = os.path.join(self.base_save_dir, "Grabaciones_RAW_EvCam")
            os.makedirs(final_cam_dir, exist_ok=True)

            self.lbl_status.setText("● GRABANDO (Staging SSD -> HDD)")
            self.btn_rec.setText("■ DETENER GRABACIÓN")
            self.btn_rec.setStyleSheet("background-color: #ff0000; color: white;")
            
            # Inyectamos las rutas y la duración del chunk directamente de la GUI
            chunk_time = self.cmb_chunk.currentData()
            if self.worker_1: self.worker_1.start_recording(self.staging_dir, final_cam_dir, "EvCam1", chunk_time)
            if self.worker_2: self.worker_2.start_recording(self.staging_dir, final_cam_dir, "EvCam2", chunk_time)
        else:
            if self.worker_1: self.worker_1.stop_recording()
            if self.worker_2: self.worker_2.stop_recording()
            self.btn_rec.setText("● GRABAR")
            self.btn_rec.setStyleSheet("color: #ff5555; font-weight: bold; border: 1px solid #555;")
            self.lbl_status.setText("● Grabación Finalizada (Trasladando a HDD...)")
    ...
    def create_recon_tab(self):
        ...
        self.lbl_recon_video = EvCamVisor()
        # Ocultamos el ROI para la reconstrucción
        self.lbl_recon_video.roi.hide()
        ...
        l_cam.addWidget(QLabel("Cámara (HAL Serial):"))
        l_cam.addWidget(self.cmb_cam_recon)
    ...
    def toggle_recon(self):
        if self.recon_worker is None:
            ...
            self.recon_worker.serial_number = self.cmb_cam_recon.currentText()
    ...
    def create_topo_tab(self):
        ...
        self.toolbar_topo.setStyleSheet("""
            QToolBar { background-color: #121212; border: none; }
            QToolButton { background-color: #cccccc; border-radius: 4px; padding: 3px; margin: 2px; }
    ...
    def process_topography(self):
        ...
        try:
            ...
            self.ax_prof.plot([0, dist_km], [elevs[0], elevs[-1]], color='#00ffaa', lw=1.5, linestyle='--', label='Línea de Vista')
    ...
    # ========================================================
    # ========================================================
    # MÉTODOS DE LA PESTAÑA IMU & GPS (ESP32 DOBLE)
    # ========================================================
    def create_imu_tab(self):
        tab = QWidget()
        main_layout = QVBoxLayout(tab)
        
        # Un layout horizontal para poner los dos ESP32 lado a lado
        split_layout = QHBoxLayout()
        
        # --- ESP32 1 ---
        panel_1 = self._create_single_imu_panel(1)
        split_layout.addWidget(panel_1)
        
        # --- ESP32 2 ---
        panel_2 = self._create_single_imu_panel(2)
        split_layout.addWidget(panel_2)
        
        main_layout.addLayout(split_layout)

        # --- SECCIÓN INFERIOR COMPARTIDA (Consola y Botón Maestro) ---
        grp_rec = QGroupBox("Adquisición de Datos IMU/GPS Maestro")
        l_rec = QHBoxLayout()
        self.chk_imu_raw = QCheckBox("Modo RAW (Volcado directo sin procesar)")
        self.chk_imu_raw.setStyleSheet("color: #ffaa00;")
        self.btn_rec_imu = QPushButton("● GRABAR AMBOS BINARIOS (.bin)")
        ...
        self.txt_imu_console.append("Módulos IMU/GPS listos. Esperando conexión.")

        main_layout.addWidget(grp_rec)
        main_layout.addWidget(QLabel("Registro del Sistema:"))
        main_layout.addWidget(self.txt_imu_console, stretch=1)
        
        self.refresh_imu_ports() 
        return tab

    def _create_single_imu_panel(self, idx):
        grp_main = QGroupBox(f"ESP32 - {idx}")
        layout = QVBoxLayout(grp_main)
        
        # 1. Conexión
        l_conn = QHBoxLayout()
        cmb_ports = QComboBox(); cmb_ports.setStyleSheet("background-color: #333; color: #fff;")
        btn_refresh = QPushButton("⟳"); btn_refresh.setFixedWidth(30)
        btn_refresh.clicked.connect(self.refresh_imu_ports)
        cmb_baud = QComboBox(); cmb_baud.addItems(["921600", "115200", "2000000"]); cmb_baud.setStyleSheet("background-color: #333; color: #fff;")
        btn_conn = QPushButton(f"CONECTAR {idx}")
        btn_conn.setStyleSheet("background-color: #00aaaa; color: black;")
        
        # Guardar referencias
        setattr(self, f"cmb_ports_{idx}", cmb_ports)
        setattr(self, f"cmb_baud_{idx}", cmb_baud)
        setattr(self, f"btn_conn_{idx}", btn_conn)
        btn_conn.clicked.connect(lambda _, i=idx: self.toggle_imu_connection(i))
        
        l_conn.addWidget(QLabel("COM:")); l_conn.addWidget(cmb_ports); l_conn.addWidget(btn_refresh)
        l_conn.addWidget(QLabel("Baud:")); l_conn.addWidget(cmb_baud); l_conn.addWidget(btn_conn)
        layout.addLayout(l_conn)

        # 2. Rendimiento
        l_stats = QHBoxLayout()
        lbl_hz = QLabel("0.0 Hz")
        lbl_hz.setStyleSheet("font-family: Consolas; font-size: 30px; color: #00ff00; font-weight: bold;")
        stats_col = QVBoxLayout()
        lbl_bps = QLabel("Bytes/s: 0"); lbl_bps.setStyleSheet("font-family: Consolas; font-size: 12px; color: #ccc;")
        lbl_err = QLabel("Errores: 0"); lbl_err.setStyleSheet("font-family: Consolas; font-size: 12px; color: #ff5555;")
        lbl_pps = QLabel("⏱️ PPS: 0"); lbl_pps.setStyleSheet("font-family: Consolas; font-size: 14px; color: #ffcc00; font-weight: bold;")
        stats_col.addWidget(lbl_bps); stats_col.addWidget(lbl_err); stats_col.addWidget(lbl_pps)
        l_stats.addWidget(lbl_hz); l_stats.addSpacing(10); l_stats.addLayout(stats_col); l_stats.addStretch()
        
        setattr(self, f"lbl_imu_hz_{idx}", lbl_hz); setattr(self, f"lbl_imu_bps_{idx}", lbl_bps)
        setattr(self, f"lbl_imu_err_{idx}", lbl_err); setattr(self, f"lbl_imu_pps_{idx}", lbl_pps)
        layout.addLayout(l_stats)

        # 3. GPS
        l_gps = QHBoxLayout()
        btn_gps = QPushButton("📍 FIJAR COORD")
        btn_gps.setStyleSheet("background-color: #4CAF50; color: white; font-weight: bold;")
        lbl_coord = QLabel("Lat: --.---- | Lon: --.----")
        lbl_coord.setStyleSheet("font-family: Consolas; font-size: 12px; color: #ffd700; font-weight: bold;")
        btn_gps.clicked.connect(lambda _, i=idx: self.fix_coordinate(i))
        l_gps.addWidget(btn_gps); l_gps.addWidget(lbl_coord); l_gps.addStretch()
        
        setattr(self, f"lbl_gps_coord_{idx}", lbl_coord)
        layout.addLayout(l_gps)
        layout.addStretch()
        
        return grp_main

    def fix_coordinate(self, idx):
        lat = getattr(self, f"latest_lat_{idx}", None)
        lon = getattr(self, f"latest_lon_{idx}", None)
        lbl = getattr(self, f"lbl_gps_coord_{idx}")
        if lat is not None and lon is not None:
            lbl.setText(f"Lat: {lat:.6f} | Lon: {lon:.6f}")
            self.log_imu(f"📍 ESP32-{idx} Fijado: Lat {lat:.6f}, Lon {lon:.6f}")
        else:
            lbl.setText("Esperando señal...")
            self.log_imu(f"⚠ ESP32-{idx}: Aún no hay paquetes 0xBBDD.")

    def store_incoming_coord(self, idx, lat, lon):
        setattr(self, f"latest_lat_{idx}", lat)
        setattr(self, f"latest_lon_{idx}", lon)

    def update_realtime_pps(self, idx, count):
        lbl = getattr(self, f"lbl_imu_pps_{idx}")
        lbl.setText(f"⏱️ PPS: {count}")
        lbl.setStyleSheet(f"font-family: Consolas; font-size: 14px; font-weight: bold; color: {'#00ffaa' if count % 2 == 0 else '#ffcc00'};")
    ...
    def refresh_imu_ports(self):
        try:
            ...
            for idx in [1, 2]:
                cmb = getattr(self, f"cmb_ports_{idx}")
                cmb.clear()
                if ports: cmb.addItems(ports)
                else: cmb.addItem("COM3")
            if ports: self.log_imu(f"✓ Puertos detectados: {ports}")
        except Exception as e:
            self.log_imu(f"❌ Error escaneando puertos: {str(e)}")

    def toggle_imu_connection(self, idx):
        worker = getattr(self, f"imu_worker_{idx}")
        cmb_ports = getattr(self, f"cmb_ports_{idx}")
        cmb_baud = getattr(self, f"cmb_baud_{idx}")
        btn_conn = getattr(self, f"btn_conn_{idx}")
        
        if worker is None:
            port = cmb_ports.currentText()
            if not port or port == "Buscando...": return
            baud = int(cmb_baud.currentText())
            new_worker = IMUWorker(port, baud)
            new_worker.log_signal.connect(self.log_imu)
            new_worker.stats_signal.connect(lambda hz, bps, err, i=idx: self.update_imu_stats(i, hz, bps, err))
            new_worker.coord_signal.connect(lambda lat, lon, i=idx: self.store_incoming_coord(i, lat, lon)) 
            new_worker.pps_signal.connect(lambda cnt, i=idx: self.update_realtime_pps(i, cnt))
            new_worker.start()
            
            setattr(self, f"imu_worker_{idx}", new_worker)
            btn_conn.setText("DESCONECTAR")
            btn_conn.setStyleSheet("background-color: #cf222e; color: white;")
            cmb_ports.setEnabled(False); cmb_baud.setEnabled(False)
            self.btn_rec_imu.setEnabled(True)
            self.btn_rec_imu.setStyleSheet("background-color: #00aaaa; color: black;")
        else:
            worker.stop()
            setattr(self, f"imu_worker_{idx}", None)
            btn_conn.setText(f"CONECTAR {idx}")
            btn_conn.setStyleSheet("background-color: #00aaaa; color: black;")
            cmb_ports.setEnabled(True); cmb_baud.setEnabled(True)
            getattr(self, f"lbl_imu_hz_{idx}").setText("0.0 Hz")
            getattr(self, f"lbl_imu_pps_{idx}").setText("⏱️ PPS: 0")
            
            if not getattr(self, "imu_worker_1") and not getattr(self, "imu_worker_2"):
                self.btn_rec_imu.setEnabled(False)
                self.btn_rec_imu.setStyleSheet("background-color: #444; color: #888;")

    def toggle_imu_recording(self):
        worker1 = getattr(self, 'imu_worker_1', None)
        worker2 = getattr(self, 'imu_worker_2', None)
        is_rec = (worker1 and worker1.recording) or (worker2 and worker2.recording)
        
        if not is_rec:
            final_imu_dir = os.path.join(self.base_save_dir, "Grabaciones_acelerometro")
            os.makedirs(final_imu_dir, exist_ok=True)
            raw_mode = self.chk_imu_raw.isChecked()
            
            started = False
            if worker1: 
                worker1.start_recording(self.staging_dir, final_imu_dir, "Ace1", raw_mode)
                worker1.chunk_duration_s = self.cmb_chunk.currentData()
                started = True
            if worker2: 
                worker2.start_recording(self.staging_dir, final_imu_dir, "Ace2", raw_mode)
                worker2.chunk_duration_s = self.cmb_chunk.currentData()
                started = True
                
            if started:
                self.btn_rec_imu.setText("■ DETENER GRABACIÓN")
                self.btn_rec_imu.setStyleSheet("background-color: #ff0000; color: white;")
                self.log_imu(f"[INI] Grabando en SSD -> HDD...")
        else:
            if worker1: worker1.stop_recording()
            if worker2: worker2.stop_recording()
            self.btn_rec_imu.setText("● GRABAR AMBOS BINARIOS (.bin)")
            self.btn_rec_imu.setStyleSheet("background-color: #00aaaa; color: black;")
            self.log_imu("[STOP] Grabación IMU finalizada.")

    def update_imu_stats(self, idx, hz, bps, errors):
        getattr(self, f"lbl_imu_hz_{idx}").setText(f"{hz:.1f} Hz")
        getattr(self, f"lbl_imu_bps_{idx}").setText(f"Bytes/s: {int(bps)}")
        getattr(self, f"lbl_imu_err_{idx}").setText(f"Errores: {errors}")

    def log_imu(self, text):
        self.txt_imu_console.append(text.strip())

    # ========================================================
    # MÉTODOS DE LA PESTAÑA DE ANÁLISIS Y CONVERSIÓN (IMU DOBLE)
    # ========================================================
    def create_analysis_tab(self):
        ...
        self.btn_load_bin_1 = QPushButton("📂 ABRIR BINARIO IMU 1")
        self.btn_load_bin_1.setStyleSheet("background-color: #00aaaa; color: black; font-weight: bold; padding: 10px;")
        self.btn_load_bin_1.clicked.connect(lambda: self.load_bin_file(1))

        self.btn_load_bin_2 = QPushButton("📂 ABRIR BINARIO IMU 2")
        self.btn_load_bin_2.setStyleSheet("background-color: #008888; color: black; font-weight: bold; padding: 10px;")
        self.btn_load_bin_2.clicked.connect(lambda: self.load_bin_file(2))
        ...
        left_layout.addWidget(self.btn_load_bin_1)
        left_layout.addWidget(self.btn_load_bin_2)
        ...
        self.imu_data = {
            1: {'ts': [], 'ax': [], 'ay': [], 'az': [], 'gx': [], 'gy': [], 'gz': [], 'pps': [], 'csv': [], 'root_atm': None},
            2: {'ts': [], 'ax': [], 'ay': [], 'az': [], 'gx': [], 'gy': [], 'gz': [], 'pps': [], 'csv': [], 'root_atm': None}
        }
        self.canvas = None
        self.toolbar = None
    ...
    def load_bin_file(self, idx):
        file_name, _ = QFileDialog.getOpenFileName(self, f"Abrir archivo Binario IMU {idx}", "", "Bin Files (*.bin);;All Files (*)")
        if file_name:
            self.txt_analysis_log.append(f"\n--- Cargando IMU {idx}: {os.path.basename(file_name)} ---")
            QApplication.processEvents() 
            self.decode_binary(file_name, idx)

    def decode_binary(self, filepath, idx):
        data = self.imu_data[idx]
        for key in data:
            if isinstance(data[key], list): data[key].clear()
        data['root_atm'] = None
        ...
        try:
            ...
            while byte_idx <= limit:
                header = struct.unpack_from("<H", raw_data, byte_idx)[0]
                
                if header == 0xBBAA:
                    try:
                        p = struct.unpack_from("<L hhhhhh", raw_data, byte_idx + 2)
                        micros = p[0]
                        ...
                        data['ts'].append(t_sec)
                        data['ax'].append(ax); data['ay'].append(ay); data['az'].append(az)
                        data['gx'].append(gx); data['gy'].append(gy); data['gz'].append(gz)
                        
                        data['csv'].append([t_sec, ax, ay, az, gx, gy, gz, 0])

                        packet_count += 1
                        byte_idx += PACKET_SIZE
                    except:
                        errors += 1; byte_idx += 1

                elif header == 0xBBCC: 
                    try:
                        p = struct.unpack_from("<L", raw_data, byte_idx + 2)
                        micros = p[0]
                        if t0 is None: t0 = micros
                        
                        t_sec = (micros - t0) / 1e6
                        data['pps'].append(t_sec)
                        data['csv'].append([t_sec, "", "", "", "", "", "", 1])
                    ...
                    try:
                        p = struct.unpack_from("<d", raw_data, byte_idx + 2)
                        unix_ts = p[0]
                        import datetime
            ...
            if packet_count > 0 or gps_count > 0:
                duration = data['ts'][-1] if data['ts'] else 0
    ...
    def plot_data(self):
        if hasattr(self, 'fig_imu'):
            plt.close(self.fig_imu)
        ...
        self.fig_imu = plt.figure(figsize=(12, 6), dpi=100)
        gs = gridspec.GridSpec(2, 2, width_ratios=[2.5, 1]) 
        self.fig_imu.patch.set_facecolor('#121212') 

        ax1 = self.fig_imu.add_subplot(gs[0, 0])
        ax2 = self.fig_imu.add_subplot(gs[1, 0], sharex=ax1)
        ax3 = self.fig_imu.add_subplot(gs[:, 1])
        ...
        colors = {
            1: {'ax': '#ff6b6b', 'ay': '#4ecdc4', 'az': '#f7fff7', 'gx': '#ff9ff3', 'gy': '#feca57', 'gz': '#54a0ff', 'pps': '#00ffaa'},
            2: {'ax': '#cc0000', 'ay': '#009999', 'az': '#cccccc', 'gx': '#cc00cc', 'gy': '#cc9900', 'gz': '#0066cc', 'pps': '#ff5555'}
        }
        ...
        for idx in [1, 2]:
            ...
            step = 1
            if len(data['ts']) > 20000:
                step = int(len(data['ts']) / 10000)
                if idx == 1: self.txt_analysis_log.append(f"Gráfico: Mostrando 1 de cada {step} puntos.")

            t = data['ts'][::step]
            ls = '-' if idx == 1 else '--'
            alpha_v = 1.0 if idx == 1 else 0.7
            c = colors[idx]
            pre = f"I{idx} "

            ax1.plot(t, data['ax'][::step], lw=0.8, ls=ls, alpha=alpha_v, color=c['ax'], label=pre+'Ax')
            ax1.plot(t, data['ay'][::step], lw=0.8, ls=ls, alpha=alpha_v, color=c['ay'], label=pre+'Ay')
            ax1.plot(t, data['az'][::step], lw=0.8, ls=ls, alpha=alpha_v, color=c['az'], label=pre+'Az')

            ax2.plot(t, data['gx'][::step], lw=0.8, ls=ls, alpha=alpha_v, color=c['gx'], label=pre+'Gx')
            ax2.plot(t, data['gy'][::step], lw=0.8, ls=ls, alpha=alpha_v, color=c['gy'], label=pre+'Gy')
            ax2.plot(t, data['gz'][::step], lw=0.8, ls=ls, alpha=alpha_v, color=c['gz'], label=pre+'Gz')

            if len(data['pps']) > 1:
                intervals = np.diff(data['pps'])
                pps_x = data['pps'][:-1]
                
                ax3.plot(pps_x, intervals, marker='o', markersize=4, linestyle=ls, color=c['pps'], lw=1, alpha=alpha_v, label=f'Deriva IMU {idx}')
        ...
        if plotted_any:
            ...
            ax3.axhline(y=1.0, color='red', linestyle=':', label='1.0s Ideal')
            ax3.set_title("Deriva (Clock Drift ESP32)", color='white', fontsize=10)
            ...
            ax1.text(0.5, 0.5, "Sin datos para graficar", color='white', ha='center', va='center')

        plt.subplots_adjust(hspace=0.1, wspace=0.25)

        self.canvas = FigureCanvas(self.fig_imu)
    ...
    def export_csv(self):
        save_path, _ = QFileDialog.getSaveFileName(self, "Guardar CSV (Se añadirá _IMU1 y _IMU2 al final)", "", "CSV (*.csv)")
        ...
        for idx in [1, 2]:
            data = self.imu_data[idx]
            if not data['csv']: continue
            
            final_path = f"{base_name}_IMU{idx}.csv"
            try:
                self.txt_analysis_log.append(f"\nExportando IMU {idx} a {os.path.basename(final_path)}...")
                QApplication.processEvents() 
                
                export_rows = []
                for row in data['csv']:
                    t_relativo = row[0]
                    if data['root_atm'] is not None:
                        t_absoluto = data['root_atm'] + t_relativo
                ...
                self.txt_analysis_log.append(f"✓ ¡Exportación IMU {idx} exitosa!")
            except Exception as e:
                self.txt_analysis_log.append(f"❌ Error al exportar IMU {idx}: {e}")

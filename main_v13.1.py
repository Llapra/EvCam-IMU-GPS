...
# Forzar PyQtGraph a usar el mismo sistema de coordenadas que NumPy (Fila-Columna)
...
class MainWindow(QMainWindow):
    ...
    def toggle_recording(self):
        ...
        if not is_recording:
            ...
            # Creación dinámica de carpetas apuntando al disco definido
            cam_dir = os.path.join(self.base_save_dir, "Grabaciones_RAW_EvCam")
    ...
    def toggle_imu_recording(self):
        ...
        if not self.imu_worker.recording and not getattr(self.imu_worker, 'armed', False):
            # Creación dinámica de carpetas destino
            ...
            if self.imu_worker.start_recording(self.staging_dir, final_imu_dir, "Ace", raw_mode):
                # Inyectamos la duración seleccionada al hilo del IMU
                self.imu_worker.chunk_duration_s = self.cmb_chunk.currentData()
                
                self.btn_rec_imu.setText("■ DETENER GRABACIÓN")
                self.btn_rec_imu.setStyleSheet("background-color: #ff0000; color: white;")
                self.log_imu(f"[INI] Grabando con Chunking de {self.imu_worker.chunk_duration_s}s ...")
        else:

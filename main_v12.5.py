        ...
        while self.running:
            try:
                ...
                for evs in mv_iterator:
                    ...
                    # 1. LÓGICA DE GRABACIÓN DIRECTA (Independiente de la visualización)
                    ...
                    if (now - last_visual_time) >= visual_period:
                        # OPTIMIZACIÓN EXTREMA: Submuestreo de eventos visuales.
                        # Obligamos a procesar solo ~10,000 eventos por frame para liberar CPU y RAM.
                        # (La grabación no se ve afectada, guarda el 100% de los datos)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Declarar el directorio base antes de inicializar la interfaz
        self.base_save_dir = os.path.join(os.getcwd(), "Grabaciones EvCam-IMU-GPS")
        ...
        # Directorio base de guardado (Por defecto el directorio actual)
        self.base_save_dir = os.path.join(os.getcwd(), "Grabaciones EvCam-IMU-GPS")
    ...
    def select_save_directory(self):
        """Define el punto de montaje/directorio en el sistema de archivos (ej. Disco Externo)."""
        directory = QFileDialog.getExistingDirectory(self, "Seleccionar ubicación de guardado (Ej. Disco Externo)")
        if directory:
            self.base_save_dir = directory
            self.lbl_save_dir.setText(self.base_save_dir)
            self.txt_log.append(f"✓ Nuevo directorio de volcado: {self.base_save_dir}")
    ...
    def create_controls_panel(self):
        ...
        self.btn_set_dir = QPushButton("📁 ASIGNAR UNIDAD DE GUARDADO")
        ...
        self.lbl_save_dir = QLabel(self.base_save_dir)
    ...
    def toggle_imu_recording(self):
        ...
        if not self.imu_worker.recording and not getattr(self.imu_worker, 'armed', False):
            ...
            # Creación dinámica de carpetas apuntando al disco definido
            imu_dir = os.path.join(self.base_save_dir, "Grabaciones_acelerometro")
